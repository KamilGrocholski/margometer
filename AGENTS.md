...
- **W10. A work round starts with the readings current.** What `frozen/` holds is evidence a guard
  ...
  observation is `deno task game:readings status` leaving a non-zero exit — a bundle fetched and
  never re-frozen, a dump past the floor it states. How a reading is refreshed is
  `frozen/AGENTS.md`'s.
