...
import { type PanelEdges, readEdgesOf, readPointsAlongBar, setDragged } from "./panel-probe.ts";
 ...
 * named and carries no line about a rest (`develop ADR 0064`). The fixture fight names one; the corpus
 ...
 * first letter, so what survives the cut still says which one is holding. **`develop ADR 0097`.**
 */
 ...
 * window — and a card is the one thing a reader asked for by pointing. **`develop ADR 0068`.**
 */
 ...
 * is not — the panel is not consulted either way (`develop ADR 0090`). At the width this suite runs at
 ...
 * on the branch that introduced it, which is why both windows are measured here (`develop ADR 0090`).
 */
test("the panel's own card follows the panel, not the window beside it", async ({ panel }) => {
    ...
    // then lies over the window is `develop ADR 0068`'s case, decided, and not what this measures.
    expect(
        card.left,
...
/**
 * The `Prowokacja` section had no card at all until `develop ADR 0098`, so both of its cells cut for
 * good: the nickname by the panel's own rule and the okrzyk by `develop ADR 0097`'s, which made it the
