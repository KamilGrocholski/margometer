...
/**
 * The whole of what the corpus grants, measured 2026-09-12: 208 announcements are followed by two
...
Deno.test("a run of the announcer's own blows is two at most, and only for three skills", () => {
    ...
    assertEquals(
        [...longer.entries()].sort(),
        [["Podwójne trafienie", 149], ["Podwójny strzał", 56], ["Struna płomienna", 3]],
...
Deno.test(
    ...
    () => {
        ...
        assertStrictEquals(bounded, 205, "what the table bounds");
...
Deno.test("a granted blow carries damage and nothing a second reading would place", () => {
    ...
    assertStrictEquals(read, 208, "every second blow the corpus holds was read");
});
 ...
 * isolates is exactly the table's own contribution — 205 blows and 136,272 points — and the three
 * it does not move are the other half of the rule showing through.
 */
Deno.test("the table reaches 205 blows, and the bound reaches three the table cannot", () => {
 ...
 * gone, 205 blows fall back into the row, and every one of them stands mid-strike — so a reader
