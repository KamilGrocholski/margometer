 ...
 * of the first is that nobody has confused them yet. None of these needs `fabricated/` to exist,
 * which it does on no machine but one that made a fight: a guard that needed the directory would
...
/** The formatter's own list, and the one every subcommand reads. */
const EXCLUSIONS = 2;
...
/**
 * ⚠️ **A permission list cannot see a formatter**, and `deno fmt` rewrites every JSON file it walks
 * to. A rewritten fabricated fight is only a nuisance; the gate going red on a file git does not
 * carry is not. Named in both lists `deno.json` carries, because the one that matters is the
 * formatter's own.
 */
Deno.test("the formatter is told to walk past a fabricated fight", () => {
    ...
    assertEquals(
        ...
        `${CONFIGURATION_FILE} excludes ${FABRICATED_DIRECTORY}/ in ${named} lists, not` +
            ` ${EXCLUSIONS}: the formatter's own and the one every subcommand reads`,
    );
});
