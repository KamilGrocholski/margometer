/**
 * `CHANGELOG.md` against the version declared and the rules its own header states: a version
 * bumped without a section would publish a release saying nothing about itself, an untyped entry
 * is invisible to a player skimming for **Zmiana**, and a second sentence is written for us. The
 * sections are cut by `tools/changelog.ts`, the reader the release workflow runs.
 */
...
const CHANGELOG = Deno.readTextFileSync(CHANGELOG_PATH);
...
/**
 * The one section the order rule arrived too late for. A section past its tag is not touched, so
 * its two entries out of order stay where a player who installed `0.7.0` saw them.
 */
...
const ENTRY_CLOSERS = ['"', ")", " "];
/**
 * The last section written before the one-sentence rule, named from the old side: the heading
 * under the boundary never moves again, where the first one written under the rule would.
 */
const SECTIONS_BEFORE_THE_RULE = "## [0.17.0]";

Deno.test("the declared version has a section, and it says something", () => {
    const section = lookupChangelogSection(CHANGELOG, DECLARED);
    assertExists(section, `${CHANGELOG_PATH} says nothing about ${DECLARED}`);
    assert(
...
Deno.test("a released section covers its own version and no neighbour", () => {
    const released = lookupChangelogSection(CHANGELOG, "0.4.0") ?? "";
...
Deno.test("the notes carry the section and say which file to click", () => {
    ...
    assertStringIncludes(
        notes,
        lookupChangelogSection(CHANGELOG, DECLARED) ?? "",
...
function lookupKindOrderFaults(text: string): string[] {
    const faults: string[] = [];
    let section = "";
    let reached = 0;
...
Deno.test("an entry from the rule down is one sentence", () => {
    ...
    assertEquals(lookupSentenceFaults(kept, null), [], "one sentence states no fault");
    const numbered = "## [1.0.0]\n\n- **Zmiana** — Panel mówi 0.17.0 zamiast 0.16.0.\n";
    assertEquals(
        lookupSentenceFaults(numbered, null),
        [],
        "a version number is not a sentence end",
    );
    const quoted = '## [1.0.0]\n\n- **Zmiana** — Panel mówi „Walka się skończyła." i tyle.\n';
    assertEquals(
        lookupSentenceFaults(quoted, null),
        [],
        "and neither is a quotation of the game's",
    );
