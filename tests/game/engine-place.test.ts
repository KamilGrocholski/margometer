...
import { getPlaceFromEngine } from "@/src/game/engine-place.ts";
...
Deno.test("the map and the tile are read off the client's own state", () => {
    const place = getPlaceFromEngine(composeEngine("Tempest", 12, 34));
...
Deno.test("a tile arrives as text as readily as a number", () => {
    ...
    assertEquals(getPlaceFromEngine(composeEngine("Tempest", "east", 1))?.x, null, "and neither");
});

Deno.test("a tile of zero is a tile somebody stands on", () => {
    const place = getPlaceFromEngine(composeEngine("Tempest", 0, 0));
...
Deno.test("what the page will not say is null, and a page saying nothing is no place", () => {
    const loading = getPlaceFromEngine({ map: { d: {} }, hero: { d: { x: 12, y: 34 } } });
    assertEquals(loading, { mapName: null, x: 12, y: 34 }, "a map mid-load still leaves a tile");
    assertEquals(getPlaceFromEngine({ map: { d: {} }, hero: { d: {} } }), null, "and none of it");
    assertEquals(getPlaceFromEngine({}), null, "an engine holding neither says nothing");
    assertEquals(getPlaceFromEngine(null), null, "and what is not an engine says nothing either");
});
...
Deno.test("a page tearing itself down is a reading of nothing, not a failure of ours", () => {
    ...
    assertEquals(getPlaceFromEngine(throwing), null, "the throw stays inside this file");
});
...
Deno.test("the client's own method for this is never called", () => {
    ...
    assertEquals(getPlaceFromEngine(engine)?.mapName, "Tempest", "the place is read");
