...
export function requireCachedHelpArticle(value: unknown, article: string): CachedHelpArticle {
    ...
    assertStrictEquals(stated, article, "a manifest names the article it was asked for");
...
function writeHelpStatusReport(article: string): void {
    ...
    assertStrictEquals(cached.article, article, "the report names the article it was asked for");
...
/** What stands over the frozen counts, exported for the reason `tools/protocol-key-table.ts`
 * gives for exporting its own. */
 ...
 *
 * What the table is **for** is `docs/protocol-keys.md`'s: a claim that the help does or does not
 * document a key is re-earned against it on every gate.
 */
...
if (import.meta.main) {
    ...
    if (command === "status") {
        ...
        console.log(
            "usage: deno task game:help [--article N] [--context N] [--count N]" +
