...
export function composeIntakeText(dump: unknown): {
  ...
  const written = {
    ...header,
    pseudonimow: getCarriedCount(header, "pseudonimow") + named.changed,
    opisow: getCarriedCount(header, "opisow") + described.removed,
  };
  return {
...
function getCarriedCount(header: Record<string, unknown>, field: string): number {
  ...
  const count = getIntegerFromValue(carried);
  if (count === null || count < 0) {
    throw new CapturedFightIntakeError(
      `the recording states \`${field}\` as something this tool cannot read as a count, ` +
        "so what an earlier redaction already did to it is unknown — writing a fresh " +
        "count over it would state that nothing had been done",
    );
  }
  return count;
}
