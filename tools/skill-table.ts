...
/**
 * The effect naming attacks a skill grants beyond its own. The game's word, spelled by the one
 * file that reads it (**N13**); what core reads is the count this derives.
 */
 ...
 * tree reads; the two figures stay here, because only two keys state one and neither is a share.
 */
 ...
 *
 * ⚠️ **A level states a value whether or not a duration stands beside it.** `add_attacks=1` is
 * written with no `@` at all, so a walk that skipped such a level would read the key and throw
 * away the only figure it carries.
 */
function readEffectFromText(text: string): SkillEffectReading | null {
    ...
    for (const level of text.slice(at + 1).split(LEVEL_SEPARATOR)) {
        ...
        const front = marked === -1 ? level : level.slice(0, marked);
        const value = getIntegerFromText(front.trim());
    ...
    assert(amounts.length <= values.length, "and an amount is one of the values read");
...
/**
 * What the table grants beyond a skill's own attack. The **fewest** stated at any level, for the
 * reason a shout's coverage is: the panel does not know the caster's skill level, so it may only
 * rely on what holds at every one of them. Measured 2026-09-12: three of the two hundred and
 * twenty-six skills state this key, each at one value across every level.
 */
export function composeGrantedBlows(
    ...
    assert(found.length <= skills.length, "no more skills grant a blow than the page served");
    assert(found.every((one) => one.blowsGrantedMinimum > 0), "and a grant is worth a blow");
    return found;
}
...
export function writeFrozenSkillTable(): {
    ...
    const blows = granted
        .map((one) =>
            `        { id: ${composeIntegerText(one.id)}, ` +
            `blowsGrantedMinimum: ${one.blowsGrantedMinimum} },`
        )
        .join("\n");
    Deno.writeTextFileSync(
        FROZEN_BLOWS_PATH,
    ...
    assert(granted.length <= skills.length, "and no more grants than there were skills");
    return {
...
if (import.meta.main) {
    ...
    if (command === "status") {
        ...
        console.log(
            `froze ${composeIntegerText(frozen.skills)} skills → ${FROZEN_PATH}, ${
                composeIntegerText(frozen.auras)
            } reaching a side → ${FROZEN_AURA_PATH}, and ${
                composeIntegerText(frozen.granted)
            } granting a blow → ${FROZEN_BLOWS_PATH}`,
        );
    } else {
