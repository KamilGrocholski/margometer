...
3. **Every event is asked whether it opens a turn**, and every event moves the standing, whether it
   opened one or not. `lookupTurnOpener` and `composeTurnStanding` in `src/core/turn-clock.ts` are
   that rule, and `tools/turn-reading.ts` imports them rather than restating them.
...
second kind**, 2026-09-25: since **develop ADR 0057** no stretch in the corpus counts over, so no
row on this table is a claim about the message it names.
