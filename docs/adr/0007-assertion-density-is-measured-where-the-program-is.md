...
S5 states an assertion density averaging at least two per function across the code. The rule was
applied to every TypeScript file in the tree, which at this commit means guards and nothing else.
...
counter incremented from zero. **S5 forbids exactly that**: an assertion a static checker can prove
can never fail, or never hold, violates the rule, which therefore cannot be satisfied by adding
unhelpful `assert(true)` statements.
...
**Keep the scope and pad to the threshold.** Rejected on the rule's own words, twice measured.
...
**Lower the threshold below two.** Rejected: two is what the rule states, and the problem was never
the number. It was counting the wrong functions.
