...
- **how many** the captures carry — occurrences of the key, not messages carrying it, because a
  message may state one twice;
- **where** they sit, strongest first: `alone in its message` is a message carrying no other key;
  `on a skill announcement` is one carrying any of the three an announcement is recognised by;
  `on a blow` is one carrying a figure of the damage family; `on a message reporting damage` is one
  carrying such a figure **or** `+oth_dmg`, which is damage no blow of its own carried; and
  `anywhere` is the floor. The strongest phrase that holds for **every** occurrence is the true one
  — where a stronger will not hold for all of them, the weaker one is what is left, and `anywhere`
  is what a key fitting none of the four gets. The last two are a chain and not a choice: every
  message on a blow reports damage, and not the other way round. Which keys an announcement and a
  blow are recognised by is `src/core/protocol-key.ts`'s reading of each key, which
  `tools/protocol-key-shape.ts` asks rather than listing them (**N13**);
- **what the key states beside itself** — `no value`, `a whole number`, `a number`, or `text`, read
  through the primitives in `libs/`, not by eye. A whole number is a number, so a key stating both
  is `a number`; a key stating no value on one occurrence and something on another is a shape this
  vocabulary cannot say, and is refused rather than filed under the nearest word.
...
omission fails. The `?dmg*` entry is the one exception and it is a **family** rather than a key: the
client has no case label for the family's members, so the register mirrors that and the guard counts
a carried key the family rule reaches as documented by it.
...
### And the counts the prose writes anyway

The line above is where a count of a key lives, because a machine re-earns it. So: **A count of
occurrences in prose names the recordings it was counted on.** A recording is evidence and never
changes, so a count scoped to one cannot go stale; a count over `captures/` goes wrong the day the
...
`tests/repository/protocol-keys.test.ts` re-earns the line against what `src/core/protocol-key.ts`
reads each key as, both ways: a key read as moving health carries the line, and a key carrying it is
one read that way. The line reaches no further than that. In particular it does **not** reach
`tests/core/health-witness.test.ts`, which names the two keys it has to excuse itself —
`healall_per`, whose share it cannot size, and `hp_per-allies`, which moves the maximum the two
percentages either side of it are read against. A figure the witness cannot add makes every later
comparison in that call wrong, and those two are the ones it cannot add.
...
`tests/repository/protocol-keys.test.ts` re-earns every token from `src/core/protocol-key.ts` **in
...
`develop:docs/adr/0056-a-fight-broken-off-is-neither-a-loss-nor-a-draw.md`.
...
position kept. **No recording carries it**, which is why this entry states no `_Shape:_` line and
why `tests/core/fight-decoder.test.ts` re-earns the absence on every run.
...
_Shape:_ 14 occurrences; on a message reporting damage; text
...
`captures/2026-08-15-tempest-grupa-vs-draugr-1-1786514810315-none.json`, against a pool of 184 680,
and before it was read the witness disagreed on every one of them and on nothing else. Reading it
closed all 12 first try. That the type is overwritten rather than extended is the same article's
table of damage over time (read 2026-08-19), and the absence of `+fire` from the client's list is
...
_Shape:_ 97 occurrences; alone in its message; text
...
_Shape:_ 129 occurrences; on a skill announcement; a whole number
...
_Shape:_ 10 occurrences; alone in its message; a whole number
...
them (`tests/core/health-witness.test.ts`). On
`captures/2026-08-25-luvia-grupa-vs-mamlambo-auto-none-0.8.1.json`, the first recording to carry it:
two occurrences stating 1724 each raise the actor's stated percentage by exactly 4.00 points of a 43
092 pool, and a third states `0` and moves nothing — the zero being the reason
`tests/core/npc-heal-rule.test.ts` exists, since a key read only above zero passes every arithmetic
check here and still loses a restoration the game reported. Every recording carrying it since reads
the same, whatever the pool: that test asks the rule of each of them and names which they are, so
how many there are is not written here.
...
_Shape:_ 171 occurrences; alone in its message; a whole number
...
`captures/2026-08-15-tempest-grupa-vs-draugr-1-1786514810315-none.json` and
`captures/2026-08-15-tempest-grupa-vs-hildur-1-1786514810315-none.json` open with a payload carrying
297 and 354 messages and no snapshot beside it, so the first snapshot sits _after_ eight casts
nothing could size. Unwinding the snapshot alone refused both outright. The messages in that opening
...
`tests/ui/panel-words.test.ts` re-earns every one against the frozen counts. **develop ADR 0073.**
...
`captures/2026-08-12-tempest-grupa-vs-draugr-2-1786514810315-none.json`, every one in the direction
of too little damage decoded. Reading this half closes all eight and opens no disagreement anywhere
else. That is the protocol's own stated percentages settling it, which is the only evidence this
file accepts for a key that moves health.
...
draws it under (**develop ADR 0077**). **Bare, without the kind**: over the dump fetched 2026-09-09,
read 2026-09-11, the nominative `absorpcja fizyczna` occurs **not once** — the article qualifies the
kind only in the oblique cases, where it is contrasting this absorption with the other two it names.
...
it carries the kind and the physical one does not (**develop ADR 0077**).
...
its sentence as `_t("msg_+of_crit")` — no `%val%` hole, which is the membership rule
`PROC_END_BY_KEY` states. The neighbouring `-blok` branch reads `_t("msg_-blok %val%",{"%val%":…})`
in the same switch, so the difference is the client's own and not an inference from our material.
...
`-legbon_cleanse` and `-legbon_glare` have, and the reason `PROC_END_BY_KEY` in
`src/core/protocol-key.ts` places all three at the target's end.
...
_Shape:_ 13 occurrences; on a blow; no value
...
_Shape:_ 17 occurrences; on a blow; no value

_Help:_ names `curse`

_Evidence:_ article view,372 at the engine name `curse` (read 2026-08-09) for the effect, its
trigger and its siblings; the shared measurement. What the recordings add is the `_Shape:_` line and
nothing beyond it: they state the flag and never whom the forgone action fell on, so the split above
rests on the help rather than on them.
...
stunning effects standing on them. So every occurrence of it belongs to the combatant who was
_struck_, not to the one who swung — the opposite of `+legbon_verycrit` beside it, on a message of
the same shape.
...
no figure and sits on the defending side, as `-evade` and `-contra` do, and where it arrives the
blow's own applied figure is stated as zero.
...
composes `_t("msg_+fastarrow")` with no `%val%` hole, which is the membership rule `PROC_END_BY_KEY`
states.
...
`_t("msg_-contra")` with no `%val%`. Measured over every recording on 2026-09-17: every occurrence
rides `+crit`, which is the trigger the article states.

⚠️ **The counterattack is not joined to it.** The blow the riposte fires arrives as its own message
like any other, and nothing in the protocol says which one it was. Reading the flag claims only that
the event happened.
...
_Shape:_ 47 occurrences; on a blow; no value
...
A special blow was kept from being made ready at all. That is what parts it from
`+superspell-dispel` above, which breaks a charge already standing: this key states that none began.
The two are also the client's own distinction — it composes them from different ids, and the
sentence behind this one names the charging rather than the blow.

_Shape:_ 1 occurrences; on a blow; no value

_Help:_ names nothing of `superspell-prevented`, `prevented`, `superspell`

_Evidence:_ production build `Bb28FQty` (read 2026-09-21), where the branch composes
...
of itself for one turn, where a critical hit and an armour pierce both fall in the same turn. Blows
carrying `+crit` and `+pierce` together are common over every recording and only the occurrence the
`_Shape:_` line counts carries this key (read 2026-09-17) — so what it reports is somebody
**holding** that effect, not the two procs meeting.
...
⚠️ **No recording carries it.** `captures/` holds `+resdmg` in quantity and neither this key nor the
two below, so the entry states no `_Shape:_` line — which `tests/tools/protocol-key-shape.test.ts`
re-earns on every run, both ways round. It is read because the client settles it and `+resdmg` was
already read: a blow narrowing the same destruction to an element would otherwise raise a defect
against a key this register explains.
...
The target's **poison resistance** destroyed by this blow, in the percentage points `+resdmg` above
states rather than the points `+acdmg` counts. It rides a critical hit, which is what the help gives
as the effect's trigger and what the material shows.

⚠️ **One letter from `+acdmg`, and a different pool.** That key empties armour; this one empties a
resistance, in another unit, and a row drawing both states them apart because the protocol does
(**N13**). Nor does any rule catch it on the way past: characters 1 to 3 of this key are `act`, so
the damage family reaches it no more than it reaches `+acdmg`.

_Shape:_ 1 occurrences; on a blow; a whole number

_Help:_ names `actdmg`

_Evidence:_ article view,372 at the engine name `actdmg` (read 2026-09-15) gives it as a passive
lowering the opponent's poison resistance once the critical-hit event has fallen, and its variable
as the number of percentage points that resistance loses — which is what puts the unit with
`+resdmg` and not with `+acdmg`. Production build `Bb28FQty` composes `msg_+actdmg %val%` on the
switch that composes `msg_+acdmg`, `msg_+acdmg_destroyed` and `msg_+resdmg` the same way, each
interpolating the value into a log slot and assigning nothing (read 2026-09-16). The one occurrence
states `1` and rides a blow carrying `+crit`, on
`captures/2026-09-16-luvia-tropiciel-vs-grupa-Bb28FQty-0.17.0.json`.
...
(`captures/2026-08-12-tempest-grupa-vs-draugr-1-1786514810315-none.json`,
`captures/2026-08-14-tempest-grupa-vs-draugr-2-1786514810315-none.json` and
`captures/2026-08-15-tempest-grupa-vs-draugr-2-1786514810315-none.json`, read 2026-08-19), which is
what a reading off one recording would have missed. Held by
`tests/core/absorption-destruction-rule.test.ts`, which groups by caster for that reason and refuses
a second distinct value inside one caster's own reports.
...
_Shape:_ 21 occurrences; on a blow; a whole number
...
over every recording sits beside `+crit` and states the same value (read 2026-09-17).
...
_Evidence:_ production build `1785244275300` against the 1 occurrence in
`captures/2026-08-06-tempest-grupa-vs-hildur-1785244275300-none.json`; article view,372 at the
engine name `holytouch` (read 2026-08-09) for the effect, its trigger and whose it is.
...
_Evidence:_ on `captures/2026-08-06-tempest-grupa-vs-hildur-1785244275300-none.json`, 1 occurrence,
naming a single combatant in the actor slot, carrying no damage, and stating the same value the 68
blow reports in that recording carry. The help documents the effect under this name — article
...
health. Every occurrence over every recording sits on a message the witness judges and agrees on at
**either** end (read 2026-09-17) — one of them reads
`447544=18.54;-10000545=48.21;+rage=340;+dmg=3745;-dmg=3171` — so the key reports no health figure
the arithmetic has to account for. `-rage` exists in the client and carries no value; the captures
...
### `sunshield_per` — decoded

The armour and the equipment healing of the announcing character raised for a run of turns, declared
on the announcement of the skill applying it and **stating no figure**. What the rise is worth is
the skill's and reaches this reader nowhere, so the declaration is counted on nobody.

⚠️ **The missing value is the client's own shape, not a hole.** `+legbon_holytouch` above is the key
that looks like a flag and is not: the client composes its sentence with a `%val%` slot no
occurrence here fills. This one is the opposite case and is worth reading beside it — production
`Bb28FQty` composes a sentence with a name in it and nowhere for a figure to go. So here it is an
occurrence **carrying** a value that would be the surprise.

_Shape:_ 1 occurrences; on a skill announcement; no value

_Help:_ names `sunshield_per`

_Evidence:_ article view,372 at the engine name `sunshield_per` (read 2026-09-15) gives it as an
active effect raising the character's armour and the healing that comes off items by a share, both
falling by a fifth of the initial rise on each of that character's turns, applied on the initiation
layer and fired on the damage layer. Production build `Bb28FQty` composes it and the bare
...
`develop:docs/adr/0010-sizing-a-share-onto-a-side.md`). By the article the reduction lands on the
casting side's opponents, and in the only recording carrying the key one of ours declares it at the
...
### `heal_per-enemies` — decoded

The share by which the **opposing** side's healing from equipment is changed, stated by itself in a
message naming the character that holds the effect. Signed, and negative wherever it occurs — the
protocol states the change, as `mana` below does.

⚠️ **It is not `lowheal_per-enemies` above.** That key reduces the healing active skills give, is
named by the article as one of the three reducing `healall_per`, and is what this meter sizes a cast
against. This one reaches the sum that comes off equipment, which no total here keeps — so it is
declared, and counted on nobody.

_Shape:_ 2 occurrences; alone in its message; a whole number

_Help:_ names `heal_per-enemies`

_Evidence:_ article view,372 (read 2026-09-15) names it among the percentage modifiers an opponent's
skills bring to bear on restored health, beside `critpoison_per`, and among the effects acting on
the sum that comes off equipment and blessings — never among what reduces `healall_per`, which is
`lowheal_per-enemies`'s job above and is stated there. Production build `Bb28FQty` composes it with
`heal_per-allies`, `hp_per-allies` and `hp_per-enemies` on **one case**, every one of the four as
`eng_game_nick_and_value_` and the key, with a name and a figure (read 2026-09-16). Both occurrences
are on `captures/2026-09-16-luvia-tropiciel-vs-grupa-Bb28FQty-0.17.0.json`, each stating `-15` and
naming one of theirs in the actor slot with no target beside it. The published table carries it on
skill 166 (`frozen/skill-durations.ts`).

### `heal_per-allies` — decoded

The same share on the other side of the ledger: the healing **our own** side takes off equipment,
raised by a skill somebody on it holds. Declared and counted on nobody, for the reason above.

⚠️ **No recording carries it.** As the set stood 2026-09-16 the corpus holds the `-enemies` half and
none of this one, so the entry states no `_Shape:_` line. It is read because the client settles it
and its sibling was already read: a message narrowing the same modifier to the caster's own side
would otherwise raise a defect against a key this register explains.

_Help:_ names `heal_per-allies`

_Evidence:_ one case, four keys, one composition — `heal_per-enemies` above carries the reading of
production build `Bb28FQty`. The article (read 2026-09-15) names this key among the modifiers a
player's or an ally's own skills bring to the same sum, which is what makes it that key's mirror
rather than a second reading of it. The published table carries it on skill 275
(`frozen/skill-durations.ts`).

### `hp_per-allies` — decoded

The share by which every member of the announcing character's side has their **health pool** raised,
stated by itself in a message naming the character that holds the effect. A pool and not health
moved: no total here counts a maximum, so the figure is declared and counted on nobody.

⚠️ **It moves the number a fight starts from.** The article is explicit that a character carrying it
opens the fight at a different health level than it stood at outside one. Nothing this meter reads
is disturbed — the health witness takes its maxima from the snapshots the payload states, which are
that fight's — but a maximum quoted from one recording as a fact about a character is exactly what
this key makes wrong, which `docs/captured-fights.md` already says of one monster for a different
reason.

_Shape:_ 1 occurrences; alone in its message; a whole number

_Help:_ names `hp_per-allies`

_Evidence:_ article view,372 at the engine name `hp_per-allies` (read 2026-09-15) gives it as a
passive raising every team member's health pool by a share of the holder's own, fired on the
initiation layer, standing to the end of the fight. Production build `Bb28FQty`: one case, four
keys, one composition, as `heal_per-enemies` above states (read 2026-09-16). The one occurrence is
on `captures/2026-09-16-luvia-tropiciel-vs-grupa-Bb28FQty-0.17.0.json`, stating `10` and naming a
paladin of theirs with no target beside it. The published table carries it on skill 276
(`frozen/skill-durations.ts`).
...
own vocabulary or not at all: the stem rule would have this key's silence tried as `per-enemies`, a
fragment the article carries throughout, so the claim could never be counted to nothing. What the
effect is comes from its three siblings and from the one case that composes all four.

_Evidence:_ production build `Bb28FQty` composes it on the case that composes `heal_per-allies`,
`heal_per-enemies` and `hp_per-allies` the same way, each with a name and a figure (read
2026-09-16). The published table carries no skill with it.
...
beside `mana` and every one stating `0`; on
`captures/2026-08-12-tempest-grupa-vs-draugr-1-1786514810315-none.json`, 4 occurrences, every one
stating `-23` and none of them beside `mana`. Which of the two a recording carries is not read off
anything here, and neither branch reaches a figure the panel draws (read 2026-09-17).
...
The total by which a monster's statistics were raised for facing a party smaller than it is set
against, stated on the monster and before anything else in the fight. Read and counted as nothing:
what the boost did is already here by other routes — the raised maximum health is in the snapshot
the roster is built from, and the raised damage arrives in every blow.

_Shape:_ 1 occurrences; alone in its message; a whole number

_Help:_ names `surpass_bonus`

_Evidence:_ article view,372 at the heading _Wzrost siły potwora dla małej grupy_ (read 2026-09-06),
which documents the mechanic under the engine names `surpass_bonus` and `surpass_treshold` and gives
the boost as `max(0, (surpass_treshold - players_amount) * surpass_bonus)` in percent, over the
monster's damage, health, armour, resistances and attack speed. So this key carries the product and
neither input: both are the monster's own and the protocol states neither. Production build
`ne0iTNdg` composes `surpass_bonus_total %val% %name%` into the battle log and assigns nothing.

**The material settles the unit, which the help alone would not.** The corpus holds the same monster
at the same level twice: ten players against it state no such key and a maximum of 43 092
(`captures/2026-08-25-luvia-grupa-vs-mamlambo-auto-none-0.8.1.json`), five state `14` and a maximum
of 49 124 (`captures/2026-09-06-luvia-grupa-5-vs-mamlambo-auto-ne0iTNdg-0.14.0.json`). 43 092 × 1.14
is 49 124.88, and the pool the second fight is met with is that figure with the fraction dropped —
so the value is a percentage, the ten-a-side fight is the unboosted pool, and what it raised is
already in the snapshot the roster is built from (read 2026-09-06).
...
Production build `53XkBRxF` composes it through the same branch its physical counterpart does. On
`captures/2026-08-26-luvia-grupa-vs-draugr-53XkBRxF-0.8.1.json` its 1 occurrence is the message that
carries that counterpart.
...
may be low, and reaches no figure. Every occurrence over every recording rides an announcement that
also carries `heal_target` and `combo-max` (read 2026-09-17).
...
Production build `53XkBRxF` composes it through `msg_removestun-allies`, interpolating nothing. On
`captures/2026-08-25-luvia-grupa-vs-draugr-none-none.json` its 1 occurrence shares its announcement
with `removeslow-allies`.
...
_Shape:_ 7 occurrences; alone in its message; a whole number
...
⚠️ **The health witness cannot judge the fight this rule was read off, and the reading does not rest
on it.** `captures/2026-08-24-tempest-tropiciel-vs-centaur-1786514810315-none.json` arrives as a
single engine call with no opening snapshot, so the replay in `tests/core/health-witness.test.ts`
seeds no running total and produces no comparison for it either way — the route `fire` and `light`
came in by is simply not available here. What stands in its place is the same body of evidence
chained from a percentage the protocol states rather than from a snapshot, measured by
...
is a wound and a reader counting the ones they left wants all of them (**develop ADR 0095**).
...
_Help:_ names `of_wound1`

_Evidence:_ production build `Cl9U89Zr` (read 2026-09-09) composes `msg_+of_wound` on the same
switch as `msg_+wound` and `msg_+critwound`, and writes no `%val%` hole into it — the switch that
composes `+injure` with one. The published help documents the weapon attribute `wound1, of_wound1`
and the type's behaviour in its table of damage over time — article `view,372` (read 2026-08-19) —
and prints no separate entry for this announcing form, as it prints none for `+wound` or
`+critwound`. Both occurrences ride blows of one player's against the monster in
`captures/2026-09-14-luvia-grupa-vs-mamlambo-auto-Cl9U89Zr-0.16.0.json`, which is the whole of the
material for this key (read 2026-09-14).
...
ticking, which is the second of the three things the wound join needs (**develop ADR 0022**); this
one carries none, so a tick and an application cannot be matched and the nearest earlier blow is a
...
Read with the entry below, which states the pair: the evidence is that entry's.
