...
`tests/game/fight-underway.test.ts` fails per recording.
...
- **where** they sit, strongest first: `alone in its message` is a message carrying no other key;
  ...
  message on a blow reports damage, and not the other way round. Which keys the first two name is
  `src/core/fight-decoder.ts`'s, spelled once there (**N13**);
...
`tests/repository/protocol-keys.test.ts` re-earns the line against `src/core/fight-decoder.ts`, both
ways: a key that decoder moves health with carries the line, and a key carrying it is one that
decoder moves health with. The line reaches no further than that. In particular it does **not**
reach `tests/core/health-witness.test.ts`, which names the two keys it has to excuse itself —
...
| `the message actor`        | the protocol states the cause in the actor slot of the message itself                                                                                    | `UNACCOUNTED_HEALTH_KEY`, and the attack family             |
| `the wound's attacker`     | an **earlier** message announced the effect and named who applied it, and the figure says which application is ticking (**ADR 0022**)                    | `WOUND_ANNOUNCEMENT_KEY` with `WOUND_TICK_KEY`              |
...
_Shape:_ 1193 occurrences; on a message reporting damage; text
...
_Shape:_ 891 occurrences; anywhere; text
...
_Shape:_ 161 occurrences; alone in its message; a whole number
...
_Shape:_ 31 occurrences; on a blow; a whole number
...
_Shape:_ 629 occurrences; on a blow; a whole number
...
_Shape:_ 73 occurrences; on a blow; no value
...
`BLOW_END_BY_PROC_KEY` states. The neighbouring `-blok` branch reads
`_t("msg_-blok %val%",{"%val%":…})` in the same switch, so the difference is the client's own and
not an inference from our material.
...
_Shape:_ 60 occurrences; on a blow; no value
...
composes `_t("msg_+fastarrow")` with no `%val%` hole, which is the membership rule
`BLOW_END_BY_PROC_KEY` states.
...
_Shape:_ 48 occurrences; on a blow; no value
...
_Shape:_ 1064 occurrences; anywhere; a whole number
...
_Shape:_ 66 occurrences; on a blow; no value
...
_Shape:_ 36 occurrences; alone in its message; a whole number
...
_Shape:_ 9 occurrences; on a blow; a whole number
...
_Shape:_ 116 occurrences; on a skill announcement; a whole number
...
_Shape:_ 44 occurrences; on a skill announcement; a whole number
...
_Shape:_ 87 occurrences; on a skill announcement; a whole number
...
_Shape:_ 165 occurrences; on a skill announcement; text
...
_Shape:_ 328 occurrences; alone in its message; text
